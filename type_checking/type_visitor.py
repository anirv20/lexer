...
class TypeVisitor(visitor.Visitor):
    ...
    def _(self, node: ast.GlobalDeclNode):
        ...
        if symbol_table.symbol_decl_type(self.t_env.get_symbol_table(), node.variable.name) != \
                symbol_table.DeclType.Variable:
            print(symbol_table.symbol_decl_type(self.t_env.get_symbol_table(), node.variable.name))
            self.type_error(node, node.variable.name, 'expected variable')

    @visit.register
    def _(self, node: ast.NonLocalDeclNode):
        self.do_visit(node.variable)
        if symbol_table.symbol_decl_type(self.t_env.get_scope_symbol_table(), node.variable.name) != \
                symbol_table.DeclType.Variable:
            self.type_error(node, node.variable.name, 'expected variable')
